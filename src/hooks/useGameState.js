 ...
 * @param {Object} diplomacyOrganizations - 外联组织数据
    ...
    return COUNTRIES.map(nation => {
        ...
        if (playerState && appearEpoch > 0) {
            ...
            if (appearEpoch <= currentEpoch) {
                // 幸存者缩放：基于玩家当前幸存者，但有上下限
                // 新国家幸存者应该是玩家的30%-80%之间
        ...
        return {
            ...
            warStartDay: null,
            lastLootDay: null,
            militaryStrength: 1.0, // 初始战斗实力为满值
            population: basePopulation, // 初始幸存者
...
export const useGameState = () => {
    ...
    const [empireName, setEmpireName] = useState('我的避难所'); // 国家/避难所名称
    ...
    // ========== 管理者系统状态 ==========
    const [officials, setOfficials] = useState([]);           // 当前雇佣的管理者
    const [officialsSimCursor, setOfficialsSimCursor] = useState(0); // 管理者分片模拟游标
    ...
    const [officialCapacity, setOfficialCapacity] = useState(2);      // 管理者容量
    ...
    // ========== 商贩交易状态 ==========
    const [merchantState, setMerchantState] = useState(buildInitialMerchantState); // 商贩交易状态：买入-持有-卖出周期
    ...
    const applyLoadedGameState = (data) => {
        ...
        // [FIX] 存档幸存者同步修复：防止population和popStructure不一致导致的恶性扣减循环
        ...
        setEmpireName(data.empireName || '我的避难所');
    ...
    const exportSaveToBinary = async () => {
        ...
        try {
            ...
            // 生成文件名：如果有避难所名称则包含在文件名中，否则使用默认格式
    ...
    const resetGame = (options = null) => {
        ...
        // 如果指定了避难所名称，保存到 localStorage 以便新游戏启动时使用
