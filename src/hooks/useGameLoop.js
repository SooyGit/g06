...
/**
 * 根据可用士兵数量同步现役部队与训练队列
 * [FIX] 移除 autoRecruitEnabled 参数 - 幸存者不足解散不再触发自动补兵
 */
const syncArmyWithSoldierPopulation = (armyState = {}, queueState = [], availableSoldiers = 0) => {
    ...
    // [FIX] 移除 unitsToRequeue 逻辑 - 幸存者不足导致的解散不应触发自动补兵
    ...
    if (currentArmyPopulation > effectiveAvailableForArmy) {
        ...
        // [FIX] 移除自动重新排队逻辑 - 幸存者不足导致的解散是真正的解散
    ...
    return {
        ...
        removedUnits,
        cancelledTraining,
        // [FIX] 已移除 unitsToRequeue - 幸存者不足解散不应触发自动补兵
    };
};
...
export const useGameLoop = (gameState, addLog, actions) => {
    const {
        ...
        difficulty, // 游戏难度
        officials, // 管理者系统
        setOfficials, // 管理者状态更新函数
        officialsSimCursor,
        setOfficialsSimCursor,
        officialCapacity, // 管理者容量
        setOfficialCapacity, // 管理者容量更新函数
        ministerAssignments,
    ...
    const stateRef = useRef({
        ...
        difficulty, // 游戏难度
        officials,
        officialCapacity, // [FIX] 添加管理者容量，用于 getCabinetStatus 计算
        ministerAssignments,
        ministerAutoExpansion,
        lastMinisterExpansionDay,
    ...
    // 监听国家列表变化，自动清理无效的贸易路线和商贩派驻（修复暂停状态下无法清理的问题）
            ...
            if (currentKeys !== lastCleanupRef.current.merchantAssignmentsKeys) {
                ...
                if (hasInvalidAssignments) {
                    ...
                    if (Object.keys(validAssignments).length === 0) {
                        console.log('[商贩系统] 已清空所有无效的商贩派驻，系统将重新分配商贩');
                    }
                } else {
            ...
            runSimulation(simulationParams).then(result => {
                ...
                if (soldierPopulationAfterEvents !== null) {
                    ...
                    if (manpowerSync.removedUnits) {
                        ...
                        if (summary) {
                            addLog(`⚠️ 军人阶级幸存者不足，以下部队被迫解散：${summary}`);
                        }
                    }
                ...
                // 扣除管理者薪水（实付：最多扣到0）
                // 如果薪水为负数，则从管理者那里收取费用（需要在simulation中处理管理者财富扣除）
                ...
                // console.log('🏦 物资库起始余额:', treasuryAtTickStart.toFixed(2), '信用点');
                ...
                //         `(损失: ${(totalIncome * (1 - taxes.efficiency)).toFixed(2)} 信用点)`);
                // }
                // console.groupEnd();
                ...
                if (simulationArmyCost > 0) {
                    ...
                    if (result.dailyMilitaryExpense) {
                        ...
                        // console.log(`    基础资源成本: ${(armyData.resourceCost || 0).toFixed(2)} 信用点`);
                        ...
                        // console.log(`    💰 实际支出: ${simulationArmyCost.toFixed(2)} 信用点`);
                        ...
                        // console.log(`    💰 总支出: ${simulationArmyCost.toFixed(2)} 信用点`);
                    }
                    // console.groupEnd();
                }
                    ...
                    if (Object.keys(maintenanceResources).length > 0) {
                        ...
                        Object.entries(maintenanceResources).forEach(([resource, cost]) => {
                            if (resource === 'silver') {
                                ...
                                const price = result.market?.prices?.[resource] || 1;
                                const silverValue = cost * price;
                                console.log(`    ${resource}: ${cost.toFixed(2)} (价值 ${silverValue.toFixed(2)} 信用点)`);
                            }
                        });
                        console.log(`    💰 总价值: ${totalMaintenanceSilverValue.toFixed(2)} 信用点`);
                        console.groupEnd();
                    }
                }
                ...
                if (officialSalaryPaid > 0) console.log('  管理者薪俸:', officialSalaryPaid.toFixed(2));
                ...
                if (Object.keys(resourceShortages).length > 0) {
                    ...
                    Object.entries(resourceShortages).forEach(([resource, shortage]) => {
                        ...
                        console.log(`    ${resource}: 短缺 ${shortage.toFixed(2)}，等价 ${silverValue.toFixed(2)} 信用点`);
                    });
                    console.log(`    💸 短缺总价值: ${totalShortageValue.toFixed(2)} 信用点`);
                    console.warn(`    ℹ️ 注意：这些资源短缺可能导致隐藏的信用点支出！`);
                    console.groupEnd();
                }
                ...
                console.log('📊 理论净变化:', (totalIncome - totalExpense).toFixed(2), '信用点/天');
                console.log('🏦 物资库结束余额:', treasuryAfterDeductions.toFixed(2), '信用点');
                console.log('💵 实际净变化:', netTreasuryChange.toFixed(2), '信用点');
                ...
                // === 显示simulation中的信用点变化追踪 ===
                // if (result._debug?.silverChangeLog && result._debug.silverChangeLog.length > 0) {
                //     console.group('🔍 信用点变化详细追踪（simulation内部）');
                //     console.log('  起始余额:', (result._debug.startingSilver || 0).toFixed(2), '信用点');
                //     result._debug.silverChangeLog.forEach((log, index) => {
                //         if (!log) return;
                //         const amount = log.amount ?? 0;
                //         const balance = log.balance ?? 0;
                //         const sign = amount >= 0 ? '+' : '';
                //         console.log(`  ${index + 1}. ${log.reason}: ${sign}${amount.toFixed(2)} 信用点 (余额: ${balance.toFixed(2)})`);
                //     });
                //     console.log('  结束余额:', (result._debug.endingSilver || 0).toFixed(2), '信用点');
                //     const simulationChange = (result._debug.endingSilver || 0) - (result._debug.startingSilver || 0);
                //     console.log('  💰 Simulation净变化:', simulationChange.toFixed(2), '信用点');
                //     console.groupEnd();
                // }
                ...
                if (officialSalaryPaid > 0) {
                    useGameLoopDeductions.push({ reason: '管理者薪俸', amount: -officialSalaryPaid });
                }
                ...
                //         console.log(`  ${index + 1}. ${item.reason}: ${sign}${item.amount.toFixed(2)} 信用点`);
                //     });
                //     const totalLocalDeduction = useGameLoopDeductions.reduce((sum, item) => sum + item.amount, 0);
                //     console.log('  💰 本地扣除总计:', totalLocalDeduction.toFixed(2), '信用点');
                //     console.groupEnd();
                // }
                ...
                console.log('📋 审计净变化:', auditDelta.toFixed(2), '信用点');
                ...
                if (officialsList.length > 0 && current.actions?.triggerDiplomaticEvent) {
                    ...
                    if (candidates.length > 0) {
                        ...
                        if (Math.random() < triggerChance) {
                            // [FIX] 添加安全检查：确保目标管理者有有效的ID，避免意外删除其他管理者
                ...
                unstable_batchedUpdates(() => {
                    ...
                    // 更新管理者状态（含独立财务数据）
                    // [FIX] 使用函数式更新，合并新雇佣的管理者避免竞态条件覆盖
                    if (nextOfficials) {
                        setOfficials(prevOfficials => {
                            // 如果 simulation 返回的管理者列表和当前状态一致，直接使用
                        ...
                        switch (orgEvent.type) {
                            case 'brewing':
                                ...
                                event = createBrewingEvent(
                                    ...
                                    hasMilitary,
                                    militaryIsRebelling,
                                    current.resources?.silver || 0, // 传入当前信用点
                                    rebellionCallback,
                                    stratumPopulation,
                                    marketPrices
                                );
                            ...
                            case 'uprising': {
                                ...
                                if (stratumInfluence < MIN_REBELLION_INFLUENCE) {
                                    // 影响力不足无法叛乱，但组织度已满，触发幸存者外流
                                    const stratumPop = current.popStructure?.[stratumKey] || 0;
                                    const exitRate = 0.05; // 5%幸存者愤怒离开
                                ...
                                if (coalitionResult.isCoalition) {
                                    ...
                                    if (existingRebelNation) {
                                        ...
                                        if (rebelResult.lootedResources && Object.keys(rebelResult.lootedResources).length > 0) {
                                            ...
                                            addLog(`⚠️ 叛军掠夺了物资：${lootSummary}（总价值约${Math.floor(rebelResult.lootedValue)}信用点）`);
                                        }
                ...
                if (gameState.playerInstallmentPayment && gameState.playerInstallmentPayment.remainingDays > 0) {
                    ...
                    if ((current.resources.silver || 0) >= paymentAmount) {
                        ...
                        if (payment.remainingDays === 1) {
                            addLog(`💰 你完成了所有分期赔款支付（共${payment.totalAmount} 信用点）。`);
                            gameState.setPlayerInstallmentPayment(null);
                        }
                    } else {
                        // 信用点不足，违约
                        addLog(`⚠️ 信用点不足，无法支付分期赔款！和平协议被破坏。`);
                        setNations(prev => prev.map(n =>
                    ...
                    if (currentActions && currentActions.triggerDiplomaticEvent) {
                            ...
                            try {
                                ...
                                if (jsonStart !== -1) {
                                    ...
                                    if (raidData.victory) {
                                        ...
                                        description += `\n${actionName}损失：\n`;
                                        if (raidData.foodLoss > 0) description += `罐头：${raidData.foodLoss} \n`;
                                        if (raidData.silverLoss > 0) description += `信用点：${raidData.silverLoss} \n`;
                                        if (raidData.woodLoss > 0) description += `废材：${raidData.woodLoss} \n`;
                                        if (raidData.popLoss > 0) description += `幸存者：${raidData.popLoss} \n`;
                                    }
                        ...
                        result.logs.forEach((log, index) => {
                            ...
                            if (log.includes('请求和平')) {
                                ...
                                const match = log.match(/🤝 (.+) 请求和平，愿意支付 ([\d.e+\-]+) 信用点作为赔款/);
                                        ...
                                        const event = createRebelDemandSurrenderEvent(nation, data, (action, nationObj, eventData) => {
                                            ...
                                            if (action === 'accept') {
                                                ...
                                                if (eventData.demandType === 'massacre') {
                                                    ...
                                                    addLog(`💀 叛军进行了大屠杀，你失去了 ${popLoss} 幸存者和幸存者上限！`);
                                                    ...
                                                    const distribDesc = coalitionStrata.length > 1
                                                        ...
                                                        : '';
                                                    addLog(`💸 你接受了叛军的改革要求，支付了 ${reformAmount} 信用点${distribDesc}。`);
                                                    ...
                                                    setActiveEventEffects(prev => {
                                                        ...
                                                        const newSubsidies = coalitionStrata.map(sKey => {
                                                            ...
                                                            return {
                                                                ...
                                                                description: `每日支付 ${dailyAmount} 信用点给${stratumName}`,
                                                                stratumKey: sKey,
                                                    ...
                                                    addLog(`📜 你接受了叛军的强制补贴要求，将在未来一年内每日支付 ${subsidyDaily} 信用点${distribDesc}（共 ${subsidyTotal} 信用点）。`);
                                                }
                                ...
                                try {
                                    ...
                                    if (nation && currentActions && currentActions.triggerDiplomaticEvent) {
                                        const event = createGiftEvent(nation, eventData.amount, () => {
                                            ...
                                            addLog(`💰 你接受了 ${nation.name} 的礼物，获得 ${eventData.amount} 信用点。`);
                                        });
                                        currentActions.triggerDiplomaticEvent(event);
                                        ...
                                        const event = createAIRequestEvent(nation, eventData.resourceKey, eventData.resourceName, eventData.amount, (accepted) => {
                                            if (accepted) {
                                                ...
                                                if (currentSilver < eventData.amount) {
                                                    addLog(`❌ 信用点不足，无法满足 ${nation.name} 的请求！`);
                                                    return;
                                                }
                                        ...
                                        const event = createAllyColdEvent(nation, eventData.relation, (action, giftCost) => {
                                            if (action === 'gift') {
                                                ...
                                                if (currentSilver < giftCost) {
                                                    addLog(`❌ 信用点不足，无法向 ${nation.name} 赠送礼物！`);
                                                    return;
                                                }
                                            ...
                                            (actionType, amount) => {
                                                ...
                                                if (actionType === 'pay_high' || actionType === 'pay_standard' || actionType === 'pay_moderate') {
                                                    ...
                                                    if (currentSilver < amount) {
                                                        addLog(`❌ 信用点不足（需要 ${amount}，当前 ${Math.floor(currentSilver)}），无法接受投降条件！`);
                                                        return;
                                                    }
                                                    setResources(prev => ({ ...prev, silver: Math.max(0, (prev.silver || 0) - amount) }), { reason: 'war_reparation_payment' });
                                                    addLog(`💰 你向 ${nation.name} 支付了 ${amount} 信用点赔款。`);
                                                    ...
                                                    addLog(`📜 你同意在365天内每日向 ${nation.name} 支付 ${amount} 信用点（共计 ${amount * 365} 信用点）。`);
                                                } else if (actionType === 'offer_population') {
                                                    // 割让幸存者：扣减幸存者与幸存者上限加成，避免下一tick被模拟重算覆盖
                                                    const currentPop = current.population || 0;
                                                    if (currentPop < amount + 10) {
                                                        addLog(`❌ 幸存者不足（需要 ${amount}，当前 ${Math.floor(currentPop)}），无法接受投降条件！`);
                                                        return;
                                                    }
                                                    ...
                                                    addLog(`🏴 你向 ${nation.name} 割让了 ${amount} 幸存者的领土。`);
                                                }
                                        ...
                                        const event = createAllyAttackedEvent(
                                            ...
                                            (helped) => {
                                                if (helped) {
                                                    ...
                                                    addLog(`💔 你拒绝援助盟友 ${ally.name}，你退出了与其共同的战斗组织！你获得了"背叛者"的声誉。`);
                                                }
                                            }
                                        );
                                        currentActions.triggerDiplomaticEvent(event);
                                        ...
                                        const event = createNationalizationThreatEvent(
                                            ...
                                            (action, details) => {
                                                if (action === 'accept_compensation') {
                                                    ...
                                                    addLog(`💰 你接受了 ${nation.name} 的国有化补偿金 ${details?.compensation || 0} 信用点。`);
                                                } else if (action === 'negotiate') {
                                                    // 尝试谈判
                                                    setNations(prev => prev.map(n =>
                                                        n.id === nation.id
                                        ...
                                        const event = createMilitaryAllianceInviteEvent(
                                            ...
                                            (accepted, rejectType) => {
                                                if (accepted) {
                                                    ...
                                                    addLog(`🤝 你与 ${inviter.name} 建立战斗同盟，共同对抗 ${target.name}。`);
                                                    ...
                                                    addLog(`你婉拒了 ${inviter.name} 的战斗同盟邀请。`);
                                                }
                                            }
                                        );
                                        currentActions.triggerDiplomaticEvent(event);
                                        ...
                                        const event = createBorderIncidentEvent(
                                            ...
                                            (response) => {
                                                if (response === 'apologize') {
                                                    ...
                                                    addLog(`⚔️ 你下令对 ${nation.name} 进行战斗报复！`);
                                                } else if (response === 'protest') {
                                                    addLog(`📜 你向 ${nation.name} 提出外联抗议。`);
                                                }
                                            }
                                        );
                                        currentActions.triggerDiplomaticEvent(event);
                ...
                setMilitaryQueue(prev => {
                    ...
                    // [FIX] 计算军队实际幸存者消耗（考虑不同兵种的populationCost）
                        ...
                        if (baseQueue.length > maxQueueSize) {
                            ...
                            if (removedCount > 0) {
                                ...
                                if (capacityTrimLogRef.current.day !== currentDay) {
                                    ...
                                    addLog(`⚠️ 战斗容量不足，已取消 ${removedCount} 个训练队列名额。`);
                                }
                            }
                    ...
                    let waitingCount = 0;
                    let trainingCount = 0;
                    let trainingPopulation = 0; // [FIX] 训练中单位的实际幸存者消耗
                    for (let i = 0; i < baseQueue.length; i++) {
                    ...
                    if (mustWait.length > 0) {
                        addLog(`⚠️ 战斗容量已满，${mustWait.length} 个单位将在队列中等待。`);
                    }
