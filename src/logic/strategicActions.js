...
export const STRATEGIC_ACTIONS = {
    ...
    crackdown: {
        ...
        effectPreview: {
            ...
            population: { value: -5, unit: '%', label: '幸存者损失', type: 'immediate' },
        },
        // 副作用列表：用于UI展示负面影响警告
        sideEffects: [
            { text: '军人满意度 -15', severity: 'warning', icon: 'Users' },
            { text: '其他阶层满意度 -5', severity: 'warning', icon: 'Users' },
            { text: '获得「镇压疲惫」：战斗力量 -20%，持续30天', severity: 'danger', icon: 'AlertTriangle' },
            { text: '获得「恐怖氛围」：生产效率 -15%，持续20天', severity: 'danger', icon: 'AlertOctagon' },
            { text: '该阶层幸存者损失约5%', severity: 'danger', icon: 'Skull' },
        ],
        // 使用建议
    ...
    divide: {
        ...
        detailedDescription: '通过挑起阶层间的矛盾来瓦解叛乱联盟。这一策略会严重打击目标阶层的组织度，但作为代价，其对立阶层（如工人↔军阀、种植员↔区长）会被激怒，组织度反而上升。适合在多个阶层同时不满时使用。',
        effectPreview: {
        ...
        // 对立阶层说明（用于UI提示）
        rivalPairsHint: {
            peasant: '区长', serf: '区长', worker: '军阀', miner: '军阀',
            lumberjack: '区长', merchant: '管理者', artisan: '商贩', soldier: '管理者',
            landowner: '种植员', capitalist: '工人',
        },
        cost: {
            culture: 200,
            silver: 500,
    ...
    bribe: {
        ...
        detailedDescription: '向该阶层发放"好处费"来临时安抚民心。费用按幸存者计算（每人10信用点）。效果立竿见影但不持久，且每次使用后成本会增加50%。适合作为短期救急方案。',
        effectPreview: {
        ...
        applicableStagesNames: ['不满', '动员中', '激进化'],
        // 成本计算说明
        costCalculationHint: '幸存者 × 10 信用点（每次使用后成本 +50%）',
        cost: {
            silverPerPop: 10,
        },
        effects: {
...
export function checkActionAvailability(actionId, stratumKey, gameState) {
    ...
    if (action.cost) {
        if (action.cost.silver && (resources?.silver || 0) < action.cost.silver) {
            return { available: false, reason: `信用点不足（需要${action.cost.silver}）` };
        }
        if (action.cost.culture && (resources?.culture || 0) < action.cost.culture) {
            return { available: false, reason: `士气点不足（需要${action.cost.culture}）` };
        }
        if (action.cost.silverPerPop) {
            ...
            if ((resources?.silver || 0) < required) {
                return { available: false, reason: `信用点不足（需要${required}）` };
            }
        }
    }
    ...
    if (action.requirements) {
        if (action.requirements.minMilitaryPower &&
            ...
            return { available: false, reason: '战斗力量不足' };
        }
