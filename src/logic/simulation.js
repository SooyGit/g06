...
export const simulateTick = ({
    ...
    difficulty, // 游戏难度设置
    officials = [], // 管理者列表
    officialsSimCursor = 0, // 管理者分片模拟游标
    ...
    const sellProduction = (resource, amount, ownerKey) => {
        // 特殊处理信用点产出：直接作为所有者收入，不进入物资库，不交税
    ...
    let totalMaxPop = 5;
    let militaryCapacity = 0; // 新增：战斗容量
    ...
    // 核心逻辑：只有阶层业主(STRATUM)才提供业主岗位，其他类型(管理者/外资/国企)不提供

    // 每个建筑的实际岗位需求（考虑外资/管理者减少业主岗位）
    ...
    if (!hasPreviousPopStructure) {
        ...
        if (diff > 0) {
            ...
            // 幸存者减少：仅从流浪者中扣除，不自动从各职业扣除（防止幸存者被吸走）
    ...
    // 注意：official 阶层不参与自由流动，人数由雇佣的管理者数决定
    ROLE_PRIORITY.forEach(role => {
        if (role === 'official') return; // 管理者不参与普通裁员逻辑
    ...
    // 管理者财富由每位管理者独立持有，不计入 wealth.official（清零以避免重复）
    wealth.official = 0;
        ...
        if (Object.keys(effectiveOps.output).length > 0) {
            ...
            for (const [resKey, totalAmount] of Object.entries(effectiveOps.output)) {
                ...
                if (isTradableResource(resKey)) {
                    ...
                    if (resKey === 'silver') {
                        // [FIX] 信用点产出需要按 owner 分配，不能直接全额进物资库
                        Object.entries(levelOutputAmounts).forEach(([lvlStr, resOutputs]) => {
                            const lvl = parseInt(lvlStr);
                            const levelBaseOutput = resOutputs[resKey] || 0;
                            if (levelBaseOutput <= 0) return;
        ...
        if (businessTaxPerBuilding !== 0 && count > 0) {
            ...
            if (totalBusinessTax > 0) {
                ...
                if (treasury >= subsidyAmount) {
                    ...
                    if (tick % 30 === 0) {
                        recordAggregatedLog(`⚠️ 物资库空虚，无法为 ${b.name} 支付营业补贴！`);
                    }
                }
            }
        }
                            ...
                            shortageTargets.forEach(target => {
                                ...
                                for (const surplus of surpluses) {
                                    ...
                                    if (importCost > logThreshold) {
                                        ...
                                        // logs.push(`?? 市场：商贩动用自有资金 ${exportValue.toFixed(1)} 信用点购入 ${exportAmount.toFixed(1)} ${fromName}，换回 ${importAmount.toFixed(1)} ${toName}。`);
                                    }
                                }
                            });
                        }
    ...
    if (hasArmyUnits || hasArmyQueue) {
        ...
        if (totalArmyCost > 0) {
            ...
            if (available >= totalArmyCost) {
                ...
                logs.push(`?? 军饷不足！应付${totalArmyCost.toFixed(0)}信用点，仅能支付${partialPay.toFixed(0)}信用点，军心不稳。`);
            }
        }
    ...
    Object.keys(STRATA).forEach(key => {
        ...
        for (const [resKey, perCapita] of Object.entries(effectiveNeeds)) {
            ...
            if (isTradableResource(resKey)) {
                ...
                // 应用管理者贪婪度修正：贪婪度直接放大财富弹性，意味着越有钱越想通过消费展示
                if (key === 'official') {
                ...
                if (amount > 0) {
                    ...
                    if (taxPaid < 0) {
                        ...
                        if ((res.silver || 0) >= subsidyAmount) {
                            ...
                            if (tick % 20 === 0) {
                                recordAggregatedLog(`物资库空虚，无法为 ${STRATA[key]?.name || key} 支付 ${RESOURCES[resKey]?.name || resKey} 消费补贴！`);
                            }
                        }
                    } else if (taxPaid > 0) {
    ...
    if (basicNeedsDeficit > 0 && workforceTotal > 0) {
        ...
        if (basicDeficitRatio > 0.1) {
            logs.push(`基础需求（食物/绷带）严重短缺，劳动效率大幅下降！`);
        }
    }
    ...
    // [OPTIMIZATION REMOVED] 移除批处理机制，所有管理者每个Tick都进行完整计算
    // 原因：
    // 1. 建筑生产、工资发放、盈利都是每个Tick更新的
    // 2. 管理者的收入（薪水、产业收益）和支出（消费、税收）应该实时计算
    ...
    const updatedOfficials = officialCount === 0 ? [] : officialList.map((official, index) => {
            ...
            if (isTradableResource(resource)) {
                ...
                if (taxPaid < 0) {
                    ...
                    if ((res.silver || 0) >= subsidyAmount) {
                        ...
                        recordAggregatedLog(`物资库空虚，无法为 管理者 支付 ${RESOURCES[resource]?.name || resource} 消费补贴！`);
                    }
                } else if (taxPaid > 0) {
        ...
        // 这里不应该再扣一次，否则会导致管理者财富被双重扣减，存款无法积累
        ...
        let totalOfficialProductionCosts = 0; // 管理者建筑的生产成本
        let totalOfficialWages = 0; // 管理者建筑的工资支出
        let totalOfficialBusinessTax = 0; // 管理者建筑的营业税
        let totalOfficialOwnerRevenue = 0; // 管理者建筑的业主收入
        ...
        Object.entries(propertySummary?.byBuilding || {}).forEach(([buildingId, count]) => {
            ...
            // [FIX] 记录管理者建筑的成本和收入到财务统计
            const finance = buildingFinancialData?.[buildingId];
            if (finance) {
                const totalCount = builds?.[buildingId] || 0;
                if (totalCount > 0) {
                    // 按比例分摊：管理者拥有的建筑数量 / 总建筑数量
        ...
        // [FIX] 记录管理者的产业成本和收入到classFinancialData
        if (classFinancialData.official) {
        ...
        if (investmentDecision && investmentDecision.cost <= investBudget && currentWealth >= investmentDecision.cost) {
            ...
            if (investmentDecision.buildingId && (eventEffectSettings?.logVisibility?.showOfficialLogs ?? true)) {
                // Log investment
                logs.push(`🏗️ 管理者${normalizedOfficial.name}投资了 ${investmentDecision.buildingId}（花费 ${Math.ceil(investmentDecision.cost)} 银）`);
            }
        }
        ...
        // 这里需要为管理者阶层使用当前管理者的薪资作为预估收入
        const estimatedClassIncome = {
            ...roleWagePayout,
            // 管理者收入使用当前管理者的薪资（如果有支付的话）加上产业收入预估
        ...
        const stanceGameState = {
            ...
            classIncome: estimatedClassIncome,  // [FIX] 使用包含管理者收入预估的数据
    ...
    // 应用管理者对出身阶层的影响力加成（方案A：直接增加“绝对影响力点数”，避免后期被阶层基数稀释）
    ...
    // 修正幸存者外流（Exodus）：愤怒幸存者离开时带走财富（资本外逃）
    Object.keys(STRATA).forEach(key => {
        const count = popStructure[key] || 0;
        if (count === 0) return;
        ...
        if (approval < 20 && influenceShare < 0.07) {
            ...
            if (leaving > 0) {
                ...
                // [FIX] 同步更新popStructure，确保幸存者真正从该阶层移除
                // 这样房屋/岗位才能空出来被新人填补
            ...
            logs.push(`${className} 阶层对政局失望，${leaving} 人离开了国家，带走了 ${(leaving * (wealth[key] || 0) / Math.max(1, count)).toFixed(1)} 信用点${shortageMsg}。`);
    ...
    if (isCleanupTick) {
        ...
        updatedNations = updatedNations.filter(n => {
            ...
            if (n.isAnnexed && (n.population || 0) <= 0) {
                logs.push(`🗑️ 国家 "${n.name}" 已被完全清除（已吞并且幸存者为0）。`);
                return false;
            }
            return true;
        });
        ...
        if (populationMigrationResult.immigrantsIn > 0 || populationMigrationResult.emigrantsOut > 0) {
            ...
            if (Object.keys(populationMigrationResult.byStratum).length > 0) {
                popStructure = applyMigrationToPopStructure(
                    ...
                    nextPopulation - netMigration  // 变化前的幸存者
                );
            }
    ...
    if ((res.food || 0) <= 0) {
        ...
        if (Math.random() > 0.9 && nextPopulation > 2) {
            ...
            // [FIX] 同步从popStructure中扣减，优先从流浪者扣
        ...
        if (needsHistory && needsHistory.length >= 5) {
            ...
            if ((lackingFood || lackingCloth) && avgSatisfaction < 0.85) {
                ...
                if (deaths > 0) {
                    ...
                    const reason = lackingFood && lackingCloth ? '食物和绷带' : (lackingFood ? '食物' : '绷带');
    ...
    if (completedTrades.length > 0) {
        ...
        if (partnerParts.length > 0 && updatedMerchantState.shouldLogMerchantTrades) {
            ...
            logs.push(`📦 商贩贸易完成: ${partnerParts.join('; ')}，${profitText}信用点`);
        }
    ...
    const activeRoleMetrics = ROLE_PRIORITY.map(role => {
        ...
        if (pop === 0) {
            ...
            // 商贩特例：优先使用当前运营收入（Net Income），忽略因进货导致的财富（Wealth）波动
    ...
    // 过滤掉 'official' 阶层，防止其参与自动转职（管理者只能通过任命产生）
    ...
    // 注意：跳过管理者阶层，因为管理者财富使用独立管理机制（在第 3306-3320 行已正确设置）
    Object.keys(STRATA).forEach(key => {
        // 跳过管理者阶层，管理者的 wealthPerCapita 在管理者模拟循环后已正确设置
        if (key === 'official') return;
    ...
    if (shouldAttemptMinisterExpansion && (tick - nextLastMinisterExpansionDay >= 10)) {
        ...
        if (bestCandidate) {
            ...
            logs.push(`🏛️ ${MINISTER_LABELS[bestCandidate.role] || bestCandidate.role} 扩建了 ${bestCandidate.building.name}（花费 ${Math.ceil(bestCandidate.silverCost)} 信用点）`);
        }
    }
    ...
    BUILDINGS.forEach(b => {
        ...
        for (let fromLevel = 0; fromLevel < maxLevel; fromLevel++) {
            ...
            // logs.push(`??? ${ownerName}自发投资了自己的产业 ${b.name} → ${upgradeName}（花费 ${Math.ceil(totalSilverCost)} 信用点）`);
    ...
    if (pendingOfficialUpgrades.length > 0) {
        pendingOfficialUpgrades.forEach(upgrade => {
            ...
            // logs.push(`??? 管理者${officialName}升级了 ${buildingId}（花费 ${Math.ceil(cost)} 银）`);
        });
    }
    ...
    // 目前 Ledger 的 taxBreakdown.xxx 代表真实转入物资库的税额（实际支付），因此这里不再二次扣除“效率损失”。
    ...
    // 腐败分配逻辑：将部分税收收入视为被贪污挪走（真实从物资库扣除），并按权重分配给管理者财富。
    // 腐败损失 = 税基 × (理论效率 - 实际效率)
    ...
    // 税收处理
    // 注意：Ledger 已将税收 (taxBreakdown.xxx) 添加到物资库并记录日志
    // 税收效率损失已通过腐败分配给管理者（第 6082-6105 行）
    // 这里只需要处理收入倍率加成（如果 incomePercentMultiplier > 1）

    // [FIX] 方案B：税收效率只影响物资库收入，不凭空增加信用点
    // 阶层已支付全额税款（在征收环节），物资库收到的是扣除腐败后的金额
    // incomePercentMultiplier 不应该凭空增加信用点，而应该在征收时就体现在 effectiveTaxModifier 中

    // 计算最终税额（用于 rates 显示）
    // 注意：这里不再乘以 incomePercentMultiplier，因为：
    // 1. 阶层已经按照 effectiveTaxModifier（包含所有加成）支付了税款
    // 2. 物资库收到的是扣除腐败后的金额（已经通过 corruptionLoss 处理）
    // 3. 不应该凭空增加信用点
    ...
    return {
        ...
        jobFill: buildingJobFill,
        jobsAvailable,
        buildingJobsRequired, // 每个建筑的实际岗位需求（考虑外资/管理者减少业主岗位）
        taxes,
        ...
        officials: updatedOfficials, // 更新后的管理者列表（含财务数据）
        // 计算有效管理者容量（基于时代、政体和科技）
        ...
        _debug: {
            ...
            startingSilver,  // tick开始时的信用点
            endingSilver: res.silver || 0, // tick结束时的信用点
