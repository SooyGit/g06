 ...
 * @param {Array} currentOfficials - 当前在任管理者列表
 * @param {number} capacity - 管理者容量上限
...
export const hireOfficial = (officialId, currentCandidates, currentOfficials, capacity, currentDay) => {
    if (currentOfficials.length >= capacity) {
        return { success: false, error: "管理者名额已满" };
    }
    ...
    const newOfficial = {
        ...candidate,
        hireDate: currentDay,
        wealth: OFFICIAL_STARTING_WEALTH,  // 管理者个人存款
...
const TECH_OFFICIAL_BONUS = {
    'early_administration': 2,  // 早期管理
    'bureaucracy': 3,           // 官僚制度
    'civil_service': 3,         // 文官制度
    'administrative_reform': 4, // 管理改革
...
/**
 * 计算单个管理者的“绝对影响力”（用于把管理者数量少的问题，通过质量/资历放大）
 * 设计目标：
...
export const calculateOfficialAbsoluteInfluence = (official, context = {}) => {
    ...
    // 出身阶层基座：
    // 1) 先给一个“阶层固有基座”（让区长/资本出身天然更有盘根错节的影响力）
    // 2) 再与“该阶层当前影响力”挂钩：阶层越强，培养/供养出的管理者天然更能撬动权力
    const stratumBase = {
        // 上层与新兴精英
        capitalist: 260,
        landowner: 240,
    ...
    // 用对数把它压到一个温和区间，避免阶层影响力巨大时把管理者绝对值推爆
    ...
    // 政体官僚容量：容量越高，官僚体系越成熟，单管理者可以撬动更大权力
    ...
    // 管理者个体天赋（旧字段兼容）：stratumInfluenceBonus 本来是百分比，这里当作“额外权威”折算
...
export const getOfficialInfluencePoints = (officials, isPaid = true, context = {}) => {
    ...
    officials.forEach(official => {
        ...
        // 轻微“派系份额”修正：大派系官僚体系更容易把管理者力量制度化
        ...
        // 2. 同时给管理者阶层加 25% 的影响力点数（基于管理者自身的影响力）
...
export const DISPOSAL_TYPES = {
    fire: {
        ...
        stabilityPenalty: 0,
        organizationBonus: 0,
        confirmText: '确定解雇此管理者？无法获取其财产，其名下产业将全部倒闭。',
    ...
    exile: {
        ...
        confirmText: '流放此管理者？将没收其一半财产，其名下产业将全部倒闭，其出身阶层会产生不满。',
    ...
    execute: {
        ...
        confirmText: '处死此管理者？将抄没全部家产，其名下产业将转交给原始业主阶层，但可能引发严重政治后果。',
 ...
 * @param {Array} currentOfficials - 当前管理者列表
 * @param {number} currentDay - 当前游戏天数
...
export const disposeOfficial = (officialId, disposalType, currentOfficials, currentDay) => {
    ...
    if (!official) {
        return { success: false, error: '未找到该管理者' };
    }
    ...
    if (!targetId) {
        ...
        return { success: false, error: '管理者ID无效' };
    }
