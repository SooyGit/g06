    ...
    routes.forEach((route, index) => {
        ...
        if (type === 'export') {
            ...
            if (importAmount >= 1 && !isForceBuy) {
                tradeLog.push(`🚢 进口 ${importAmount.toFixed(1)} ${RESOURCES[resource]?.name || resource} 从 ${nation.name}：商贩国外购 ${foreignPurchaseCost.toFixed(1)} 信用点，国内售 ${domesticSaleRevenue.toFixed(1)} 信用点（税 ${tradeTax.toFixed(1)}），商贩赚 ${merchantProfit.toFixed(1)} 信用点。`);
            }
        }
    });
