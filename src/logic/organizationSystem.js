...
export const STRATUM_ORGANIZATION_MULTIPLIER = {
    // 上层阶级
    capitalist: 2.4,    // 军阀 (原2.0)
    landowner: 1.8,     // 区长 (原1.5)
    official: 1.5,      // 管理者 (原1.2)
    engineer: 1.4,      // 工程师 (原1.0)
    // 中层阶级
    merchant: 1.5,      // 商贩 (原1.1)
    soldier: 1.5,       // 军人 (原1.2)
    ...
    miner: 1.2,         // 矿工 (原0.9)
    // 底层阶级
    peasant: 1.1,       // 种植员 (原0.8)
    lumberjack: 1.1,    // 拾荒者 (原0.8)
    serf: 0.9,          // 难民 (原0.6)
    unemployed: 0.4,    // 流浪者 (原0.2)
};
...
export const COALITION_REBELLION_CONFIG = {
    ...
    COALITION_BONUS: 0.1,          // 联合叛军幸存者加成 (10%)
};
...
const buildDriverContext = (stratumKey, {
    ...
    // 当该阶层没有幸存者时，直接返回空上下文，避免“幽灵阶层”积累组织度
    if (population <= 0) {
        return {
            driverScore: 0,
...
export function updateAllOrganizationStates(
    ...
    Object.keys(STRATA).forEach(stratumKey => {
        ...
        // 末日降临（epoch 0）禁止任何阶层积累组织度，防止早期叛乱
