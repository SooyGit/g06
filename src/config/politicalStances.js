...
export const POLITICAL_ISSUES = {
    ...
    military: { name: '战斗', icon: 'Sword', description: '战斗力量和国防' },
    ...
    trade: { name: '物资交换', icon: 'Handshake', description: '商业活动' },
    ...
    efficiency: { name: '效率', icon: 'Zap', description: '管理效能' },
...
const CONDITION_TYPES = {
    ...
    population_above: {
        ...
        text: (params) => `幸存者 ≥ ${params.threshold}`,
    },
    population_below: {
    ...
    epoch_at_least: {
        ...
        text: (params) => {
            const epochNames = ['末日降临', '废墟拾荒', '堡垒建设', '军阀割据', '病毒研究', '疫苗突破', '重建文明', '新世界'];
    ...
    resource_price_above: {
        generate: (market) => {
            ...
            const resourceNames = {
                food: '罐头', wood: '废材', stone: '石材', iron: '铁',
                cloth: '布匹', tools: '工具', copper: '铜', ale: '净水', coal: '燃油',
                delicacies: '鲜肉', furniture: '装甲板', fine_clothes: '防护服', spice: '稀有物资'
            };
    ...
    inflation_above: {
        generate: (market) => {
            ...
            const resourceNames = {
                food: '罐头', wood: '废材', stone: '石材', iron: '铁',
                cloth: '布匹', tools: '工具', copper: '铜', coal: '燃油',
                delicacies: '鲜肉', furniture: '装甲板', ale: '净水', fine_clothes: '防护服', spice: '稀有物资'
            };
            const resource = resources[Math.floor(Math.random() * resources.length)];
            // 基于当前市场价格生成合理阈值
            const currentPrice = market?.prices?.[resource] || 1;
...
/**
 * 为管理者生成带动态条件的政治立场
 * @param {string} sourceStratum - 管理者出身阶层
...
export function assignPoliticalStance(sourceStratum, epoch, market = null) {
    ...
    // 注意：移除了 stratum_approval_below，因为"希望某阶层满意度低"只适合激进立场（种植员起义派、无政府主义等）
