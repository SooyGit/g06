...
export const OFFICIAL_EFFECT_TYPES = {
    ...
    trade_bonus: {
        ...
        weight: 15,
        costMultiplier: 1.4,
        description: (val) => `物资交换利润 +${(val * 100).toFixed(0)}%`,
    },
    ...
    tax_efficiency: {
        ...
        weight: 15,
        costMultiplier: 1.5,
        description: (val) => `配给效率 +${(val * 100).toFixed(0)}%`,
    },
    ...
    income_percent: {
        ...
        weight: 12,
        costMultiplier: 1.5,
        description: (val) => `配给收入 +${(val * 100).toFixed(0)}%`,
    },
    ...
    max_pop: {
        ...
        weight: 10,
        costMultiplier: 1.0,
        description: (val) => `幸存者上限 +${(val * 100).toFixed(0)}%`,
    },
    ...
    population_growth: {
        ...
        weight: 10,
        costMultiplier: 1.1,
        description: (val) => `幸存者增长 +${(val * 100).toFixed(0)}%`,
    },
    ...
    research_speed: {
        ...
        weight: 12,
        costMultiplier: 1.4,
        description: (val) => `研究产出 +${(val * 100).toFixed(0)}%`,
    },
    ...
    military_bonus: {
        ...
        weight: 10,
        costMultiplier: 1.0,
        description: (val) => `战斗力量 +${(val * 100).toFixed(0)}%`,
    },
    ...
    military_upkeep: {
        ...
        weight: 10,
        costMultiplier: 1.1,
        description: (val) => `战斗维护 ${(val * 100).toFixed(0)}%`,
    },
    ...
    diplomatic_bonus: {
        ...
        weight: 8,
        costMultiplier: 1.0,
        description: (val) => `每日外联关系 +${val.toFixed(1)}`,
    },
    ...
    diplomatic_cooldown: {
        ...
        weight: 6,
        costMultiplier: 0.9,
        description: (val) => `外联冷却 ${(val * 100).toFixed(0)}%`,
    },
};
...
export const OFFICIAL_DRAWBACK_TYPES = {
    ...
    corruption: {
        ...
        weight: 15,
        description: (val) => `腐败：配给损失 ${(val * 100).toFixed(0)}%`,
    },
    ...
    diplomatic_incident: {
        ...
        weight: 8,
        description: (val) => `外联关系衰减 +${val.toFixed(1)}/日`,
    },
    ...
    research_slowdown: {
        ...
        weight: 8,
        description: (val) => `研究速度 ${(val * 100).toFixed(0)}%`,
    },
...
/**
 * 根据管理者属性生成效果
 * @param {Object} stats - 管理者属性 { administrative, military, diplomacy, prestige }
...
const NAME_STYLES = {
    ...
    GERMANIC: {
        ...
        last: ['霍亨索伦', '哈布斯堡', '维特尔斯巴赫', '末日', '俾斯麦', '克劳塞维茨', '梅特涅', '瓦格纳', '施特劳斯', '海涅', '歌德', '席勒',
 ...
 * @param {string} sourceStratum - 管理者出身阶层（用于偏好加权）
 ...
 * 生成随机管理者
 * @param {number} epoch - 当前时代
 * @param {Object} popStructure - 当前幸存者结构 { stratumKey: population }
...
const EFFECT_TYPE_WEIGHTS = {
    // 经济类：高价值但不要太极端
    incomePercent: 1.4,      // 降低：配给收入
    taxEfficiency: 1.3,      // 降低：配给效率
    tradeBonus: 1.2,         // 降低：物资交换利润
...
export const generateRandomOfficial = (epoch, popStructure = {}, classInfluence = {}, market = null, rates = null) => {
    ...
    // 管理能力 (Administrative): 20-80基础，根据阶层调整
    ...
    // 目标范围: 15 ~ 4000 银/日
    // 设计目标：
    // - 废墟拾荒 (epoch 1): 20-150 银/日
    // - 铁器时代 (epoch 3): 50-400 银/日
    // - 重建文明 (epoch 6): 150-1500 银/日
