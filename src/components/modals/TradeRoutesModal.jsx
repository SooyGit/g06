            ...
            <div className="col-span-5 flex items-center justify-end gap-2">
                ...
                <div className="flex items-center justify-end gap-2">
                    ...
                    <button
                        ...
                        {...(!disabledInc ? longPressIncrement : {})}
                        disabled={disabledInc}
                        title={nation.isAtWar ? '战争中不可派驻' : (maxWithNation === 0 ? '关系敌对不可派驻' : (value >= maxWithNation ? '已达该国派驻上限' : (remainingMerchants <= 0 ? '没有可用商贩' : '点击增加 1，长按派驻到最大值')))}
                    >
                        <Icon name="Plus" size={14} />
                    </button>
                </div>
...
const TradeRoutesModal = ({
    ...
    const calculateTradeOpportunity = (nation, resourceKey, type) => {
        ...
        if (type === 'export') {
            ...
            return {
                ...
                status: possibleVolume < 0.1 ? '对方无货' : merchantMargin <= 0 ? '商贩无利' : '可交易',
    ...
    const tabs = [
        { id: 'assignments', label: '派驻商贩', shortLabel: '派驻', count: assignedTotal, icon: 'Users' },
                        ...
                        <div className="space-y-2">
                            <div className="p-3 rounded-lg bg-amber-900/20 border border-amber-500/20 text-xs text-gray-300">
                                <div className="flex items-center justify-between gap-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="font-semibold text-amber-100">商贩派驻</span>
                                    </div>
                                    <div className="font-mono text-[11px]">
                                        总商贩: <span className="text-amber-300">{merchantCount}</span>
                                ...
                                <div className="mt-1 text-[10px] text-gray-400">
                                    派驻到某国的商贩越多，每回合越倾向在该国寻找最赚钱且最能修复供需缺口的交易。交战国自动无法贸易。
                                </div>
                                ...
                                <div className="mt-2 text-[10px] text-gray-400 space-y-0.5">
                                    <div>
                                        <span className="text-red-300 font-semibold">倾销</span>：出口时允许无视对方需求；结算时对外售价按<span className="font-mono">外价×0.6</span>（折价），通常会造成外联关系下降。
                                    </div>
                                    <div>
                                        <span className="text-purple-300 font-semibold">强买</span>：进口时允许无视对方供给；结算时对外进价按<span className="font-mono">外价×1.3</span>（溢价/加价），通常会造成外联关系下降。
                                    </div>
                                    <div className="text-green-300">
                                        若双方签订了<span className="font-semibold">开放市场</span>，则倾销/强买不会产生外联惩罚。
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-3 rounded-lg bg-gray-800/30 border border-white/5">
                                <div className="flex items-center justify-between gap-2">
                                    ...
                                    <div className="text-[10px] text-gray-500">多选 · 影响商贩选品权重</div>
                                </div>
                            ...
                            <div className="hidden sm:grid grid-cols-12 gap-2 items-center px-3 py-2 bg-white/5 text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-wider border border-white/5 rounded-lg">
                                ...
                                <div className="col-span-3 text-right">派驻商贩</div>
                            </div>
                            ...
                            {pendingTrades.length === 0 ? (
                                <div className="space-y-3">
                                    <div className="text-center py-6 text-gray-500">
                                        ...
                                        <p className="text-xs text-gray-600 mt-1">派驻商贩到其他国家后，他们会自动发起贸易</p>
                                    </div>
                                    ...
                                    <div className="p-3 rounded-lg bg-red-900/20 border border-red-500/30 text-xs">
                                        ...
                                        <div className="mt-2 pt-2 border-t border-red-500/20 text-[10px] text-gray-400">
                                            ...
                                            <ul className="list-disc list-inside mt-1 space-y-0.5">
                                                {merchantCount <= 0 && <li className="text-red-300">商贩数量为0</li>}
                                                {assignedTotal <= 0 && <li className="text-red-300">没有派驻商贩到任何国家</li>}
                                                {assignedTotal > 0 && merchantCount > 0 && (
