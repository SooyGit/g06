...
const getTargetName = (target, type) => {
    ...
    const categoryNames = { gather: '采集', industry: '工业', civic: '民用', military: '战斗' };
    if (categoryNames[target]) return categoryNames[target];
    if (target === 'silver') return '信用点';
    if (target === 'food') return '罐头';
    if (target === 'culture') return '士气';
...
const OfficialCardInner = ({
    ...
    const formatEffect = (type, target, value) => {
        ...
        switch (type) {
            ...
            // 幸存者
            case 'maxPop': description = `幸存者上限 ${pct(value)}`; break;
            case 'populationGrowth': description = `幸存者增长 ${pct(value)}`; break;

            // 研究/士气
            case 'researchSpeed': description = `研究产出 ${pct(value)}`; break;
            case 'cultureBonus': description = `士气产出 ${pct(value)}`; break;
            ...
            case 'militaryUpkeep': isGood = value < 0; description = `战斗维护 ${pct(value)}`; break;
            ...
            // 外联
            case 'diplomaticBonus': description = `外联关系 ${isPositive ? '+' : ''}${formatScalar(value)}/日`; break;
            ...
            // 外联事件
            case 'diplomaticIncident': isGood = value < 0; description = `外联关系衰减 +${value.toFixed(1)}/日`; break;

            // 外联冷却
            case 'diplomaticCooldown': isGood = value < 0; description = `外联冷却 ${pct(value)}`; break;
            ...
            <div className="mb-2">
                ...
                <div className="grid grid-cols-4 gap-1">
                    ...
                    <div className="flex flex-col items-center p-1 bg-gray-900/40 rounded border border-blue-800/30">
                        ...
                        <span className="text-[8px] text-gray-500">管理</span>
                    ...
                    <div className="flex flex-col items-center p-1 bg-gray-900/40 rounded border border-red-800/30">
                        ...
                        <span className="text-[8px] text-gray-500">战斗</span>
                    ...
                    <div className="flex flex-col items-center p-1 bg-gray-900/40 rounded border border-green-800/30">
                        ...
                        <span className="text-[8px] text-gray-500">外联</span>
