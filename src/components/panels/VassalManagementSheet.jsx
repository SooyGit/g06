...
const TRADE_POLICY_OPTIONS = [
    ...
    },
    {
        id: 'preferential',
        title: '优惠准入',
        description: '你的商贩享有优先贸易权，关税减免50%',
        effects: '维持现状（默认）',
        effectColor: 'text-gray-400',
    },
    {
        id: 'monopoly',
        title: '垄断贸易',
        description: '强制附庸所有贸易通过你的商贩，关税全免',
        effects: '独立倾向增速+30%',
        effectColor: 'text-red-400',
    },
];
        ...
        <div className="p-4 bg-gray-800/30 rounded-lg border border-gray-700/40">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="flex justify-between">
                    <span className="text-gray-400">幸存者:</span>
                ...
                <div className="flex justify-between">
                    <span className="text-gray-400">战斗义务:</span>
...
const PolicyTab = memo(({ nation, onApplyPolicy, officials = [], playerMilitary = 1.0 }) => {
    ...
    // 根据附庸类型过滤可用的外联控制选项 - All available now
        ...
        <div className="space-y-4">
            ...
            <div>
                <h3 className="text-sm font-bold text-white mb-2 flex items-center gap-1.5">
                    ...
                    治理政策
                    <span className="text-[10px] text-gray-500 ml-1">（影响附庸的管理管理方式）</span>
                </h3>
                <div className="space-y-2">
                ...
                <h3 className="text-sm font-bold text-white mb-2 flex items-center gap-1.5">
                    ...
                    {totalControlCost > 0 && (
                        <span className="text-xs text-amber-400 ml-2">
                            (每日成本: {formatNumberShortCN(totalControlCost)} 信用点)
                        </span>
                    )}
                </h3>
                            ...
                            >
                                    ...
                                    <div className="flex items-center justify-between mb-1.5">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-xs text-amber-300 font-medium">
                                                {formatNumberShortCN(dynamicCost)} 信用点/天
                                            </span>
                                            {/* 点击提示图标（非总督） */}
                                            {!isGovernor && (
                                                <Icon
                                ...
                                {measure.id === 'governor' && (
                                    <div className="px-3 pb-3 space-y-2 border-t border-gray-700/50">
                                        {/* Official Selector */}
                                        <div className="pt-2">
                                            <label className="text-[10px] text-gray-400 mb-1 block">指派管理者:</label>
                                                ...
                                                >
                                                    <span className="truncate">
                                                        {(() => {
                                                            ...
                                                            if (!selectedId) return '-- 选择管理者 --';
                                                            const selected = officialsById.get(selectedId);
                                                            if (!selected) return '未知管理者';
                                        ...
                                        {governorEffectiveness && (
                                            <div className="text-[10px] space-y-0.5 mt-1 p-2 bg-gray-800/50 rounded">
                                                <div className="text-gray-400">
                                                    ...
                                                    {' '}(威望:{governorEffectiveness.stats.prestige} 管理:{governorEffectiveness.stats.admin} 战斗:{governorEffectiveness.stats.military})
                                                </div>
            ...
            <div className={`p-2 rounded-lg border text-xs ${diplomaticControl === 'autonomous'
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold">
                        {diplomaticControl === 'autonomous' ? '自主外联' :
                            diplomaticControl === 'puppet' ? '傀儡外联' : '引导外联'}
                    </span>
                </div>
                <div className="text-[10px] mt-1 opacity-70">
                    {diplomaticControl === 'autonomous'
                        ? '附庸可自主进行外联，无需审批，您也无法下达指令'
                        : diplomaticControl === 'puppet'
                            ? '完全控制附庸外联，所有行动需要您的批准'
                            : '附庸外联需要您的审批，您也可以主动下达指令'}
                </div>
            </div>
                                ...
                                >
                                    ...
                                    {(selectedAction === 'join_org' ? joinableOrgs : leavableOrgs).map(org => (
                                        <option key={org.id} value={org.id}>
                                            {org.name} ({org.type === 'military_alliance' ? '战斗同盟' : '经济同盟'})
                                        </option>
                                    ))}
                                </select>
                            ) : (
...
export const VassalManagementSheet = memo(({
    ...
    playerPopulation = 1000000,  // NEW: 宗主国幸存者
    onApplyVassalPolicy,
    onDiplomaticAction,
    ...
    const tabs = [
        ...
        { id: 'diplomacy', label: '外联审批', icon: 'Globe', badge: pendingCount > 0 ? pendingCount : null },
    ];
