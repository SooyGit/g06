    ...
    const getSummaryText = () => {
        if (result.isRaid) {
            ...
            if (result.victory) {
                ...
                const lossDetails = [];
                if (result.foodLoss > 0) lossDetails.push(`罐头-${result.foodLoss}`);
                if (result.silverLoss > 0) lossDetails.push(`信用点-${result.silverLoss}`);
                if (result.woodLoss > 0) lossDetails.push(`废材-${result.woodLoss}`);
                if (result.popLoss > 0) lossDetails.push(`幸存者-${result.popLoss}`);
