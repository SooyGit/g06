    ...
    return createPortal(
        ...
        >
            ...
            <div className="bg-gray-900/50 rounded px-2 py-1.5 mb-2">
                ...
                <div className="text-xs mt-2 pt-2 border-t border-gray-700">
                    ...
                    <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className="text-yellow-300">{formatNumberShortCN(calculateUnitExpense(unit, market?.prices || {}, epoch, militaryWageRatio), { decimals: 2 })} 信用点/日</span>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-gray-900/50 rounded px-2 py-1.5 mb-2">
                ...
                <div className="flex justify-between text-xs">
                    <span className="text-gray-400">幸存者占用</span>
...
const MilitaryTabComponent = ({
    ...
    buildings = {}, // 新增：建筑列表，用于计算战斗容量
    nations = [],
    day = 0, // 当前游戏天数，用于计算战斗行动冷却
    selectedTarget,
    onRecruit,
    onDisband,
    ...
    const getRecruitDisabledReason = (unit, count = 1) => {
        ...
        if ((resources.silver || 0) < silverCost) {
            return `信用点不足：需要 ${formatNumberShortCN(silverCost, { decimals: 1 })} 信用点`;
        }

        // 检查战斗容量
        if (totalArmyCount + count > militaryCapacity) {
            return `战斗容量不足（${totalArmyCount}+${count}/${militaryCapacity}），需要建造更多训练营`;
        }
...
<p className="text-base font-bold text-white">{totalUnits}</p>
                            ...
                            <div className="bg-gray-700/50 p-3 rounded">
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className="text-xs text-gray-400">幸存者占用</span>
                                </div>
<p className="text-base font-bold text-cyan-400">{totalPopulationCost}</p>
                            ...
                            <div className="bg-gray-700/50 p-3 rounded">
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className="text-xs text-gray-400">战斗容量</span>
                                </div>
...
<p className="text-base font-bold text-white">{militaryQueue.length}</p>
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-700">
                            ...
                            <div className="flex items-center gap-2 text-sm">
                                ...
                                <span className="text-yellow-300 font-mono">-{totalWage.toFixed(2)} 信用点</span>
                            </div>
                            {/* 军费分解显示 */}
...
<div className="glass-ancient p-3 rounded-lg border border-ancient-gold/30">
                            ...
                            <div className="flex flex-wrap items-center gap-3 text-xs text-gray-400 mb-3">
                                ...
                                {activeNation && (
                                    <div className="flex flex-wrap gap-3">
                                        ...
                                        <span>战斗实力：{Math.floor((activeNation.militaryStrength ?? 1.0) * 100)}%</span>
                                        <span>幸存者：{Math.floor(activeNation.population || 1000)}</span>
                                        <span className={(() => {
                    ...
                    {totalUnits === 0 && (
                        <p className="text-xs text-yellow-400 mt-2">⚠️ 你需要先招募军队才能发起战斗行动</p>
                    )}
                </div>
            )}
